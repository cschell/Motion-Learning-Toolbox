...
def test_fix_controller_mapping(switched: bool):
    ...
    hmd_positions_dict = {
        ...
        "left_hand_pos_x": np.random.normal(loc=-1 * -((switched-0.5)*2), scale=3, size=n_samples),
        ...
        "right_hand_pos_x": np.random.normal(loc=1 * -((switched-0.5)*2), scale=3, size=n_samples),
    ...
    fixed_positions = fix_controller_mapping(hmd_positions,
    ...
    assert np.all(hmd_positions == fixed_positions.rename(columns=rename_mapping)[hmd_positions.columns])